...
<body>
				...
				.ready(
						function() {
							$("#qnaBtn").click(function() {
								location.href = "/inquery/InqueryWriteForm.do";
							});
	...
	<div class="row">
		<font size="5pt">나의 문의내역</font>&nbsp; <font color="gray">집사님께서
			문의하신 1:1문의 내역입니다.</font>
		<hr />
		<%--검색기능 --%>
		<div id="inquerySearch">
				...
				<table summary="검색">
					...
					<tr>
						<td id="btd1"><label>검색조건</label> <select id="search"
							...
							placeholder="검색어를입력하세요" /> <input type="button" class="btn btn-primary"" value="검색"
							id="searchData" /></td>
		...
		<div>
			<form id="detailForm">
				...
				<table class="table table-striped">
					...
					<%-- 데이터 출력 --%>
					<c:choose>
						<c:when test="${ not empty inquery }">
							<c:forEach items="${ inquery }" var="inquery" varStatus="status">
								<tr>
									<td>${ inquery.i_number }</td>
									<td><a
										href="/inquery/InqueryDetail?i_number=${ inquery.i_number }">${ inquery.i_title }</a>
									</td>
									<td>${inquery.i_date }</td>
									<td>${ inquery.i_whether }</td>
								</tr>
							</c:forEach>
						</c:when>
						<c:otherwise>
							<tr>
								<td colspan="4">작성하신 문의내역이 존재하지 않습니다.</td>
							</tr>
						</c:otherwise>
					</c:choose>
				</table>
		...
		<table>
			<tr>
				<td><input type="button" value="문의하기" id="qnaBtn"></td>
			</tr>
		</table>
