...
$(function() {
	...
	$("#addressModify").on("click",function(){
		$.ajax({
			url:"/myorder/addressmodify",
			type:"post",
			data:{"o_address":$("#o_address").val(),
			"o_number":<%=request.getParameter("o_number")%>
			},
...
<body>
<div><label class="control-label">배송주소 변경</label></div>
				<div>
					...
					<input type="text" id="sample6_address" placeholder="주소" class="form-control"
						required="required" readonly="readonly"> <input class="form-control"
						type="text" id="sample6_address2" placeholder="상세주소"><input
						type="hidden" name="o_address" id="o_address">
				<button id="addressModify">배송지변경하기</button>
				</div>
</body>
</html>