...
public class MailController {
	...
	public List<MemberVO> Id(@RequestParam("email") String email) {
		...
		List<MemberVO> idList = service.findAccount(email);
	...
	public String Id(@RequestParam("m_email") String email, @RequestParam("m_id") String id) {
		...
		try {
			...
			mvo = service.findPassword(mvo);
			StringBuffer sb = new StringBuffer();
			System.out.println(email);
			if (mvo.getM_id() != null && !mvo.getM_id().equals("") || id == mvo.getM_id()
					|| email == mvo.getM_email()) {
				...
				sb.append("<div><a href=\"http://localhost:8080/member/passwordmodifi?m_id=" + mvo.getM_id()+"&m_number="+mvo.getM_number()
						+ "\">비밀번호 변경</a></div>");
				sb.append("</body>");
				sb.append("</html>");
